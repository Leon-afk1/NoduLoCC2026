...
class AsymmetricFocalLoss(nn.Module):
    """Asymmetric focal loss (ASL) pour déséquilibre sévère.

    Utilise des gammas différents pour les positifs et négatifs :
    - gamma_neg (défaut 4) : écrase les faux négatifs faciles des 60k no-findings
    - gamma_pos (défaut 0.5) : garde le gradient sur les nodules rares

    Cela force le modèle à privilégier le rappel (recall) sur la précision.
    Réf : Ridnik et al. (2021) "Asymmetric Loss For Multi-Label Classification"
    """
    ...
    def __init__(
        ...
        self.clip = float(clip)          # probability margin pour éviter les gradients nuls
        self.pos_weight = pos_weight
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        # Probability shifting sur les négatifs uniquement (ASL paper §3.2)
        # Utilise BCE logit (numériquement stable) avec logits shiftés
        prob_neg_shifted = (prob + self.clip).clamp(max=1.0)
        logits_neg_shifted = torch.log(prob_neg_shifted.clamp(min=1e-7) / (1.0 - prob_neg_shifted).clamp(min=1e-7))

        # BCE numériquement stable via F.binary_cross_entropy_with_logits
        bce_pos = F.binary_cross_entropy_with_logits(logits, targets, reduction="none")
        bce_neg = F.binary_cross_entropy_with_logits(logits_neg_shifted, targets, reduction="none")
        # Pour les positifs on garde les logits originaux, pour les négatifs les shiftés
        bce = torch.where(targets > 0.5, bce_pos, bce_neg)

        # Focal terms asymétriques
        pt_pos = prob
        pt_neg = 1.0 - prob
        focal_pos = torch.pow(1.0 - pt_pos, self.gamma_pos)
        focal_neg = torch.pow(1.0 - pt_neg, self.gamma_neg)
...
def _build_loss_fn(cfg: dict[str, Any], train_df: pd.DataFrame, device: torch.device) -> nn.Module:
    """Build training loss from config (`bce`, `focal`, or `asl`)."""
    ...
    if loss_name == "asl":
        return AsymmetricFocalLoss(
            gamma_pos=float(loss_cfg.get("asl_gamma_pos", 0.5)),
            gamma_neg=float(loss_cfg.get("asl_gamma_neg", 4.0)),
            clip=float(loss_cfg.get("asl_clip", 0.05)),
            pos_weight=pos_weight,
        )

    raise ValueError("train.loss.name must be one of: bce, focal, asl")
...
def _set_optimizer_lr(optimizer: torch.optim.Optimizer, lrs: list[float]) -> None:
    """Applique un LR par groupe de paramètres (backbone, head)."""
...
def _build_optimizer(
    ...
    model: nn.Module,
    base_lr: float,
) -> tuple[torch.optim.Optimizer, list[list[float]]]:
    """Construit un AdamW avec LRs par couche (layer-wise LR optionnel).

    Retourne (optimizer, [backbone_lrs, head_lrs]) où chaque liste
    contient un LR par époque.
    """
    weight_decay = float(cfg["train"].get("weight_decay", 1e-4))
    epochs = int(cfg["train"].get("epochs", 1))
    layer_wise = bool(cfg["train"].get("layer_wise_lr", False))

    # Groupes de paramètres : backbone d'un côté, CBAM + pool + head de l'autre
    backbone_params = list(model.backbone.parameters())
    head_params = (
        list(model.cbam.parameters())
        + list(model.pool.parameters())
        + list(model.head.parameters())
    ) if hasattr(model, "cbam") else list(model.head.parameters())

    if layer_wise:
        lr_backbone = float(cfg["train"].get("lr_backbone", base_lr * 0.1))
        lr_head = float(cfg["train"].get("lr_head", base_lr))
    else:
        lr_backbone = base_lr
        lr_head = base_lr
    ...
    # Deux schedules cosinus indépendants, même forme mais amplitudes différentes
    backbone_lrs = _build_epoch_lrs(cfg, epochs=epochs, base_lr=lr_backbone)
    head_lrs     = _build_epoch_lrs(cfg, epochs=epochs, base_lr=lr_head)
    return optimizer, [backbone_lrs, head_lrs]
...
def _run_single_training(
    ...
    optimizer, (backbone_lrs, head_lrs) = _build_optimizer(cfg, model, base_lr)
    epochs = int(cfg["train"].get("epochs", 1))
    ...
    for epoch in range(1, epochs + 1):
        epochs_ran = epoch
        lr_bb   = float(backbone_lrs[epoch - 1])
        lr_head = float(head_lrs[epoch - 1])
        _set_optimizer_lr(optimizer, [lr_bb, lr_head])
        ...
        if val_loader is None:
            metrics = {"train_loss": float(train_loss), "lr_backbone": lr_bb, "lr_head": lr_head}
        ...
        if not _show_progress_bars():
            if val_loader is None:
                ...
                print(
                    ...
                    f"pr_auc={float(metrics.get('avg_precision', float('nan'))):.4f} "
                    f"prec={float(metrics.get('precision', 0.0)):.4f} "
                    f"rec={float(metrics.get('recall', 0.0)):.4f} "
    ...
    # --- Résumé final sur le meilleur checkpoint ---
    if best_outputs is not None and val_loader is not None:
        y_true_best, y_prob_best, _ = best_outputs
        # Cherche le meilleur threshold sur la val
        best_thr_info = find_best_f1_threshold(
            y_true_best,
            y_prob_best,
        ...
        final_metrics = classification_metrics(y_true=y_true_best, y_prob=y_prob_best, threshold=best_thr)
        sep = "=" * 60
        print(f"\n{sep}")
        print(f"RÉSUMÉ FINAL  [{log_prefix}]  best_epoch={best_epoch}  threshold={best_thr:.3f}")
        print(sep)
        print(f"  ROC-AUC        : {final_metrics['auc']:.4f}")
        print(f"  PR-AUC         : {final_metrics['avg_precision']:.4f}")
        print(f"  F1             : {final_metrics['f1']:.4f}")
        print(f"  Precision      : {final_metrics['precision']:.4f}")
        print(f"  Recall (Sens.) : {final_metrics['recall']:.4f}")
        print(f"  Specificity    : {final_metrics['specificity']:.4f}")
        print(f"  NPV            : {final_metrics['npv']:.4f}")
        print(f"  Accuracy       : {final_metrics['accuracy']:.4f}")
        print(f"  Confusion mat. : TP={int(final_metrics['tp'])}  FP={int(final_metrics['fp'])}  "
              f"TN={int(final_metrics['tn'])}  FN={int(final_metrics['fn'])}")
        n_pos = int(y_true_best.sum())
        n_neg = int((y_true_best == 0).sum())
        print(f"  Val set        : {n_pos} nodules / {n_neg} no-finding")
        print(sep + "\n")
